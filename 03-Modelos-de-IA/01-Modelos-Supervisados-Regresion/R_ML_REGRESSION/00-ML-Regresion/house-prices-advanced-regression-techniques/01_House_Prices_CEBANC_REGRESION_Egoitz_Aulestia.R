...
# Modificar la función de limpieza para preservar SalePrice
...
sum(is.na(full_data_clean))  # Debería ser 0
...
sum(is.na(test_clean))  # Debería ser 0
...
submission <- data.frame(Id = test$Id, SalePrice = predictions)
write.csv(submission, "catboost_submission_1129_01.csv", row.names = FALSE)
...
# Paso 2: Escalado de las variables numéricas
...
# Escalar las variables numéricas en train y test
train_clean <- scale_numeric(train_clean)
test_clean <- scale_numeric(test_clean)
